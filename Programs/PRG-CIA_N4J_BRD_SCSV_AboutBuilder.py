...
#01 match_cia_name(company): return foundFlag,CompanyName => Scan Cia name as tokens to search Neo4j + User Interaction
#02 def test_acronym_token(token,limit): return limitFlag,ciaMatchHits => checks if Neo4j Hits > than user set limit
#03 def adjust_cia_name(name): return CompanyName => Adjusts auxilary words that do not need Capital letters
#04 def correct_spelling_company_name(cia_name): return cia_name => Checks mispelled words in Company Names
#05 def clean_company_name(cia_name): return cia_name /* Removes Companies registration status suchas S.A., etc
#06 def check_cnpj_format(cnpj): return okFlag => cheks if Cia CNPJ data looks like a CNPJ number
#07 def find_marker_cnpj(clean_response,marker): return cnpjFlag,ciaCNPJ => scan a string to find a cnpj format sequence
#08 def init_acronyms_file(file,header): return fio,win,wout => Sets the csv readers/writers to start acronym processing
#09 def restore_acronym_set(file): return acronymSet,len(acronymSet) => Process file containg acronyms to a set
#10 def open_acronyms_import_read(file): return okFlag,fi,win => Checks file existence Open file csv.reader
#11 def getAbout_chatGPT(cia_name): return about => Ask chatGPT about a company
#12 def cleanReplyAbout(reply): return cleanReply => Removes CR/NL so about one one_line - replaces <'> by <^> and <;> by <&>
#//
...
#02 def n4j_get_companies(): return ciaList_len,ciaList => Query to N4J to download all companies in FixInc Database
#03 def n4j_check_cia_match(cia_name): => Checks if cia_name matches perfectly company already in the N4j database
#04 def n4j_correct_company_name(company_name): return correctCiaName => Checks spelling with N4J FixInc Database
#05 def n4j_get_acr_match(token): return ciaList_len,ciaList => Query N4J for CiaNames that includes token
#06 def n4j_change_cia_name(name, new_name): return ciaList_len,ciaList => Set new_name as cia_name in the N4J database
#07 def n4j_build_query_merge_cia(record_list): return cypher_script => build query to upload new Cia into N4J Database
#08 def n4j_get_company_no_about(session): return cia_name => returns one company where about =#X#
#09 def n4j_push_about(session,cia_name,about): return responseList => store about in N4J database
#//
...
#02a brd_inoperative_list => BRD meaningless about responses
#02b brd_inoperative_list_len => len(brd_inoperative_list)
#03a brd_inoperative_set =>
#03b brd_inoperative_set_len =>
#04a brd_platitudes_list => BRD Platitudes about responses:
#04b brd_platitudes_list_len => len(brd_platitudes_list)
#05 def BRD_CiaAbout(cia_name): return troubleFlag,ciaAbout => Prompts BRD for an about of a company
#06  def BRD_cleanReplyAbout(reply): return cleanReply => Removes special characters + BRD Noise
#07  def brd_cia_cnpj(cia_name): return troubleFlag,cnpjFlag,ciaCNPJ => Prompts BRD for CNPJ of a company
#08  def brd_check_traffic_trouble(reply): return troubleFlag => Checks if BRD is operational (network accessible)
#09  def BRD_checkAbooutPertinence(about):return okFlag => check if BRD reply is non operative
#10  def brd_filter_platitudes(about): return about => remove BRD boilerplate platitudes
#//

import ModFin_GPT as Mgpt
### Module-FixIncome - GPT related Routines  20230803 -v1 as Mgpt
#01  GPT_KEY_API => connection key to activate chatGPT API
#02a GPT_INOPERATIVE_LIST => List of useless sentences used by chatGPT
#02b GPT_INOPERATIVE_LIST_LEN = len(GPT_INOPERATIVE_LIST_LEN)
#03  def gpt_get_about(cia_name): return about => Activates chatGPT prompting for an about
#04  def gpt_clean_about(reply): return cleanReply => Replace some characters + remove chatGPT boilerplate text
#//

import ModGen_UserInteractionRoutines as Musr
### FixInc User Interaction Routines - 20230803-v2
#01 def usr_hit_limit(): return userResp  Asks user maximum number of hits
#02 def usr_token_feedback(company,token): return userResp  User feedback on a token: Acronym,CommonWord
#03 def usr_confirm_company(company): return foundFlag,company_name  Company not in Neo4j so what do?
#04 def USR_hitListChoice(company,hitlist): return selectFlag,CompanyName  Select Neo4j potential Hits or discard?
#05 def usr_company_confirmation(company): return userResp => Confirmation for new Company in the database
#06 def usr_company_go_nogo(company): return userResp => shows about and ask for UI confirmation or discard Cia
#07 def usr_rectify_company_name(company): return company_name => ask user to rectify company name
#08 def usr_confirm_cia_highest_hit(cvmName,n4j_name): return userResp => cvmCiaName highest hit:accept,rename or ignore?
#09 def usr_confirm_acronym(acronym,hits): return userResp  Acronym Hits level accept/discard?
#10 def usr_confirm_common_word(word): return userResp  Confirmation for a Common Word
#11 def usr_start_resume(): return userResp  [S]tart processing from scratch or [R]esume processing
#12 def usr_cia_cnpj(company): return userResp => askUser for company CNPJ
#13 def usr_cia_about(company): return userResp => askUser for company about
#14 def usr_rescue_about(cia_name): return userResp,ciaAbout => askUser to manually rescue about
#15 def usr_check_about(company,about): return userResp,clean_about => about looks fishy ask user feedback
#16 def usr_rescue_cnpj(cia_name): return userResp,ciaCNPJ => askUser to manually rescue CNPJ
#//

import ModGen_ManipulatingPythonStructures as Mpst
#// Module-General - Gestion de Structures
#01 def take_second(elem): return elem[1]  => take second element for sort
#02 def build_dict (word_list): return dict_words_occurrences => Builddictionnary [word:number of occureences] from a chain
#03 def pairlist_to_dict(entry_list): return dictionary => Take double entry entry_list and transform it in a dictionnary
#04 def dict_to_pairlist(dictionary): return entry_list => Take dictionnary and transform it in a pair entry key entry_list
#05 def short_list_elements(entry_list,n): return shorted_list => Select N first elements of a sorted entry_list
#06 def get_words(dictionary,lang,count): return words => Retrieve key words in dictionnary
#07 def list_average(entry_list): return avg => Calculate average of values in a entry_list
#//

import ModGen_FilesOperations as Mfil
#// Module-General - Files Operations
#01 def fpath(directory,filename): return path /# Crée le path pour un directory de data avec la filename
#02 def open_file_reader(filePath): return okFlag,fi,win /# Checks file existence - set reader utf8 newline '' delimiter ';'
#03 def open_file_writer(filePath): return okFlag,fo,wout /# Checks file existence - set writer utf8 newline '' delimiter ';'
#04 def openFile_ReadWriter(filePath): return okFlag,fio,win,wout /# Checks file - set R/W utf8 newline '' delimiter ';'
#05 def open_csv_reader(filePath): return okFlag,fi,win /# Checks file existence - set csv.reader utf-8 delimiter ';'
#06 def open_csv_writer(filePath) /# Checks file existence - set csv.writer utf-8 delimiter ';' newline = ''
...
#10 def restore_set_jason(filePath): return okFlag,localSet /# Check file existence - Restore Set stored in a Jason file
#11 def restore_dict_jason(filePath): return okFlag,jasonDict /# Check file existence - Restore Dict stored in a Jason file
#12 def read_oneline(fr): return one_line /# read new one_line
#13 def initialize_log_file (file,hdr): return wout,fo => setup Log file to write including header Line
#//
...
#02 def items_comma_converter(one_line): return liste,len(liste) => Converts a one_line with commas into a entry_list
#03 def items_separator_converter(one_line,separator): return liste,len(liste) => Converts a one_line with separator char into a entry_list
#//
...
# CiaCNPJ;cia_name;SubIndustryCode;IndustryCode;SectorCode;Region;Fitch;SP;Moody;Group;about;aboutBRD
...
while inputLine := fi.readline():
    ...
    if ciaCNPJ == '#X#':
        ...
        trafficTroubleFlag,brdFlag,brdCNPJ = Mbrd.brd_cia_cnpj(ciaName)
        if trafficTroubleFlag == 'True' or brdFlag == 'False':
            ...
            userResp,uiCNPJ = Musr.usr_rescue_cnpj(ciaName)
            if userResp == 'Stop':
...
# we are going now to request BRD for an about
    if ciaAboutBRD == '#X#':
        ...
        time.sleep(pauseTimer)
        trafficTroubleFlag,ciaAboutBRDCrude = Mbrd.brd_cia_about(ciaName)
        ...
        if trafficTroubleFlag == 'True':
            ...
            # we bypass automation and ask for UI help
            userResp,uiAboutBRD = Musr.usr_rescue_about(ciaName)
            ...
            aboutBRDFlag = Mbrd.brd_check_about_pertinence(ciaAboutBRDCrude)
            if aboutBRDFlag == 'False':
                ...
                ciaAboutBRD = Mbrd.brd_filter_platitudes(ciaAboutBRDCrude)
    ...
    if userContinue == 'True':
        ...
        # print(f"\n===V1-DataBuilderUpgraded=> For Company <{cia_name}> BRD provided an about:\n {ciaAboutBRD}  ")
    else:
