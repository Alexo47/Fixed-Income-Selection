...
# !! Pending Bugs
# 1 LogHeader is flawed - comma separated characters and not words hdrLog must be a list -CORRECTED
...
def readFile_cleanAbout(f):
    flag, about_file = Mfil.openFile_Reader(f)
    if flag == 'False':
...
files_names_FilePath = Mgv.filesDir + "/" + files_names_FileName
files_names = files_names_FilePath + '.txt'
files_names_Flag, files_names_fi = Mfil.openFile_Reader(files_names)
...
aboutFilePath = Mgv.filesDir + "/" + aboutFileName
...
while inputLine := fi.readline():
    ...
    if ciaAbout == '#X#':  # is empty then we will request BARD
        ...
        brdFlag, brd_ciaAbout = Mbrd.BRD_CiaAbout(ciaName)
        if brdFlag == 'False':
